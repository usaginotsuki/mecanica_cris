...
class latexPrint extends Controller
{
    public function engranajeConicoIg()
    {
        ...
        $tmpfname = preg_replace('/\\.[^.\\s]{3,4}$/', '', $basetmpfname . '.tex');
        rename($basetmpfname, $tmpfname);
        $tmpDir = storage_path('app/public');
        chmod($tmpfname, 0755);
        rename($tmpfname, $tmpfname . '.tex');
        File::put($tmpfname, $pdf);
       
        $uploadedFileUrl = cloudinary()->uploadFile($tmpfname)->getSecurePath();
        return redirect('https://www.overleaf.com/docs?snip_uri='.$uploadedFileUrl);
